...
import model.ComparatorDistance;
...
public class PriorityQueue {
	...
	if (firstElement != null) {
		...
		if (firstElement.getNextElement() != null) {
		    if (c.compare(firstElement.getNextElement().getValue(), element.getValue()) >= 0) {
	    ...
	    // Si le edge est le plus de grand de toute la file, on le met en dernier
	    if (ajoute == false) {
		tmp.setNextElement(element);
	    }
	} else {
    ...
    /**
     *
     * @return prends le maillon qui a été ajouté en premier, le supprime de la
     * chaîne et le retourne
     */
    public Element pop() {
	if (reversed) {
	    reverse();
    ...
    /**
     *
     * @return Le maillon qui a été ajouté en premier, sans le supprimer de la
     * chaîne.
     */
    public Element peek() {
	if (reversed) {
